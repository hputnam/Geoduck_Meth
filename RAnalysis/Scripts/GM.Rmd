...
#sig.GO.D10 <- read.csv("RAnalysis/Output/sig.GO.D10.csv", row.names = 1)
#sig.GO.D135 <- read.csv("RAnalysis/Output/sig.GO.D135.csv", row.names = 1)
#sig.GO.D145 <- read.csv("RAnalysis/Output/sig.GO.D145.Interactions.csv", row.names = 1)
...
write.csv(byGO_long , file = "RAnalysis/Output/Shelly_GO_annot.csv")
...
XX<-  byGO_long %>% filter(ontology=="BP") %>%
...
#Persist.GO.Sig <- ggarrange(PersistGO.BP,PersistGO.MF, ncol=2, common.legend = TRUE, legend="bottom")
ggsave(file="RAnalysis/Figs/GOslim_BP_facetsXprotein.pdf", XX, width = 5, height = 16, units = c("in"))
...
byGO_long_term_gene_rank <- byGO_long_term_gene %>% group_by(term_gene) %>% slice(which.min(order))
...
XXb<-  byGO_long_term_gene %>% filter(ontology=="BP", TP!= "Persistent") %>%
...
#Persist.GO.Sig <- ggarrange(PersistGO.BP,PersistGO.MF, ncol=2, common.legend = TRUE, legend="bottom")
ggsave(file="RAnalysis/Figs/GOslim_BP_facetsXterm", XXb, width = 10, height = 16, units = c("in"))




#subset GO slims to reduce heatmap size and focus on rapid-inducible vs. persistent

XXc<-  byGO_long_term_gene %>% filter(ontology=="BP", TP!= "Persistent",GO.Slim.Term %in% c("cell organization and biogenesis","signal transduction", "transport", "developmental processes", "protein metabolism")) %>%
  ggplot(., aes(x = TP, y = order2)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
...
ggsave(file="RAnalysis/Figs/GOslim_BP_facetsXterm_subset_long.pdf", XXc, width = 8, height = 10, units = c("in"))


XXd <- byGO_long_term_gene %>% filter(ontology=="BP", TP!= "Persistent",GO.Slim.Term %in% c("cell organization and biogenesis","signal transduction", "transport", "developmental processes", "protein metabolism")) %>%
  ggplot(., aes(x = TP, y = order2)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_wrap(factor(GO.Slim.Term, levels = c("cell organization and biogenesis","signal transduction", "developmental processes", "protein metabolism", "transport")) ~ .,nrow = 3,scales = "free_y",labeller = label_wrap_gen(width = 3, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
  strip.text.y = element_text(angle=0, size = 12),
  strip.text.x = element_text(size = 20),
  ...
  axis.title.y = element_blank())+ scale_y_continuous(breaks = unique(byGO_long_term_gene$order2),
    labels = unique(byGO_long_term_gene$term_gene),expand = c(0,0)) 
ggsave(file="RAnalysis/Figs/GOslim_BP_facetsXterm_subset.pdf", XXd, width = 15, height = 10, units = c("in"))





XXe<-  byGO_long_term_gene %>% filter(ontology=="MF",TP!= "Persistent") %>%
  ggplot(., aes(x = TP, y = order2)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ .,scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
...
ggsave(file="RAnalysis/Figs/GOslim_MF_facetsXterm.pdf", XXe, width = 8, height = 20, units = c("in"))
