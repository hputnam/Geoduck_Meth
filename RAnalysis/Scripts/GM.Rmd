...
Continuous Temperature data - Apex
...
Fig.CG.size <- ggplot(CG.Shell.size, aes(x=Day, y=avg.Arel, group=Treatment)) + 
  ...
  annotate("text", x=1, y=3.3, label = "b", size = 3) + #add text to the graphic for posthoc letters
  annotate("text", x=1.2, y=3.48, label = "b", size = 3) + #add text to the graphic for posthoc letters
  xlab("Treatment") +
  ylab(expression(paste("Relative Size"))) +
  ylim(0.5,3.5) +
  theme_bw() + #Set the background color
...
ggsave(file="RAnalysis/Figs/Geoduck_Size.pdf", Figure1.Size, width = 10, height = 4, units = c("in"))
ggsave(file="RAnalysis/Figs/Geoduck_Size.png", Figure1.Size, width = 10, height = 4, units = c("in"))
...
GoSlims <- read.csv("RAnalysis/Data/Genome/GO-GOslim.sorted", header=FALSE, sep="\t", na.string="NA", stringsAsFactors = F, skip=0)
...
GOslim.Time <- GO.slim.wall[order(GO.slim.wall$over_represented_pvalue),]
write.csv(GOslim.Time , file = "RAnalysis/Output/GOslim_Time.csv")
sig.GOslim.Time <- GOslim.Time %>%
  filter(over_represented_pvalue <0.07 | under_represented_pvalue<0.07)
...
GOslim.D10 <- GO.slim.wall[order(GO.slim.wall$over_represented_pvalue),]
write.csv(GOslim.D10 , file = "RAnalysis/Output/GOslim_D10.csv")
sig.GOslim.D10 <- GOslim.D10 %>%
  filter(over_represented_pvalue <0.09 | under_represented_pvalue<0.09)
...
GOslim.D135 <- GO.slim.wall[order(GO.slim.wall$over_represented_pvalue),]
write.csv(GOslim.D135 , file = "RAnalysis/Output/GOslim_D135.csv")
sig.GOslim.D135 <- GOslim.D135 %>%
  filter(over_represented_pvalue <0.09 | under_represented_pvalue<0.09)
...
GOslim.D145 <- GO.slim.wall[order(GO.slim.wall$over_represented_pvalue),]
write.csv(GOslim.D145 , file = "RAnalysis/Output/GOslim_D145.csv")
sig.GOslim.D145 <- GOslim.D145 %>%
  filter(over_represented_pvalue <0.09 | under_represented_pvalue<0.09)
...
D135.trans.app <- unique(D135.slim.genes.5 %>% filter(as.character(D135.slim.genes.5$gene) %in% D135.sig.annot$gene))
D135.trans.app <- left_join(D135.trans.app, D135.sig.annot, by="gene")

D135.goi <- rbind(D135.cytoskel.act,D135.cytoskel,D135.nuc_acid_bind,D135.trans.app)
...
D145.trans.app <- unique(D145.slim.genes.6 %>% filter(as.character(D145.slim.genes.6$gene) %in% D145.sig.annot$gene))
D145.trans.app <- left_join(D145.trans.app, D145.sig.annot, by="gene")

D145.goi <- rbind(D145.nuc_acid_bind,D145.trans.app)
...
write.csv(GOslim.INT_ALL , file = "RAnalysis/Output/GOslim_INT_ALL.csv")
```
