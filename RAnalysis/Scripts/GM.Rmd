...
high <- MD.Time %>%	
  filter(gene== Time.Amb.sig$gene[2])
...
sig.GO.Time$term <- factor(sig.GO.Time$term, levels = sig.GO.Time$term[nrow(sig.GO.Time):1])

#plot significantly enriched GO terms by Slim Category
TimeGO <-  ggplot(sig.GO.Time, aes(x = ontology, y = term)) + 
...
ggsave(file="RAnalysis/Figs/TimeGO.pdf", TimeGO, width = 7, height = 10, units = c("in"))
...
sig.GO.D10$term <- factor(sig.GO.D10$term, levels = sig.GO.D10$term[nrow(sig.GO.D10):1])

#plot significantly enriched GO terms by Slim Category
D10GO <-  ggplot(sig.GO.D10, aes(x = ontology, y = term)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ ., scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
ggsave(file="RAnalysis/Figs/GO.D10.pdf", D10GO, width = 7, height = 12, units = c("in"))
