...
GO.data <- GO.data[,c(2,16)]
splitted <- strsplit(as.character(GOs$GO.IDs), ";") #slit into multiple GO ids
GO.IDs <- data.frame(v1 = rep.int(GOs$gene, sapply(splitted, length)), v2 = unlist(splitted)) #list all genes with each of their GO terms in a single row 
GO.IDs <-unique(GO.IDs$v2)
write.csv(GO.IDs, "RAnalysis/Output/GO.IDS_tosearch.csv") #save to file

GO.annot <- read.csv("RAnalysis/Output/GO_details.csv", header=TRUE, sep=" ", na.string="NA", stringsAsFactors = F) #read in gene ontology information
which(GO.annot$class == 'GOSlimTerm')
colnames(GO.annot)[1] <- "category"
GO.annot$namespace <- gsub("biological_process","BP",GO.annot$namespace ) #remove extra characters
GO.annot$namespace <- gsub("molecular_function","MF",GO.annot$namespace ) #remove extra characters
GO.annot$namespace <- gsub("cellular_component","CC",GO.annot$namespace ) #remove extra characters
unique(GO.annot$namespace)
```
...
ALL.Time <-enriched.GO.05 %>% 
  arrange(-numDEInCat)
ALL.Time$category <- as.character(ALL.Time$category)
str(ALL.Time) #there is a space before " GO:XXXX" so they are not merging correctly
ALL.Time$category <- gsub(" GO:","GO:",ALL.Time$category ) #remove extra characters
str(ALL.Time)
str(GO.annot)

ALL.Time <- merge(ALL.Time,GO.annot, by="category" )
ALL.Time <- ALL.Time[order(-ALL.Time$numDEInCat),]
str(ALL.Time)
MF.Time <- subset(ALL.Time, namespace=="MF")
MF.Time <- MF.Time[order(-MF.Time$numDEInCat),]
CC.Time <- subset(ALL.Time, namespace=="CC")
CC.Time <- CC.Time[order(-CC.Time$numDEInCat),]
BP.Time <- subset(ALL.Time, namespace=="BP")
...
ALL.D10 <-enriched.GO.05 %>% 
  arrange(-numDEInCat)
ALL.D10$category <- as.character(ALL.D10$category)
str(ALL.D10) #there is a space before " GO:XXXX" so they are not merging correctly
ALL.D10$category <- gsub(" GO:","GO:",ALL.D10$category ) #remove extra characters
str(ALL.D10)
str(GO.annot)

ALL.D10 <- merge(ALL.D10,GO.annot, by="category" )
ALL.D10 <- ALL.D10[order(-ALL.D10$numDEInCat),]
str(ALL.D10)
MF.D10 <- subset(ALL.D10, namespace=="MF")
MF.D10 <- MF.D10[order(-MF.D10$numDEInCat),]
CC.D10 <- subset(ALL.D10, namespace=="CC")
CC.D10 <- CC.D10[order(-CC.D10$numDEInCat),]
BP.D10 <- subset(ALL.D10, namespace=="BP")
...
ALL.D135 <-enriched.GO.05 %>% 
  arrange(-numDEInCat)
ALL.D135$category <- as.character(ALL.D135$category)
str(ALL.D135) #there is a space before " GO:XXXX" so they are not merging correctly
ALL.D135$category <- gsub(" GO:","GO:",ALL.D135$category ) #remove extra characters
str(ALL.D135)
str(GO.annot)

ALL.D135 <- merge(ALL.D135,GO.annot, by="category" )
ALL.D135 <- ALL.D135[order(-ALL.D135$numDEInCat),]
str(ALL.D135)
MF.D135 <- subset(ALL.D135, namespace=="MF")
MF.D135 <- MF.D135[order(-MF.D135$numDEInCat),]
CC.D135 <- subset(ALL.D135, namespace=="CC")
CC.D135 <- CC.D135[order(-CC.D135$numDEInCat),]
BP.D135 <- subset(ALL.D135, namespace=="BP")
...
ALL.D145 <-enriched.GO.05 %>% 
  arrange(-numDEInCat)
ALL.D145$category <- as.character(ALL.D145$category)
str(ALL.D145) #there is a space before " GO:XXXX" so they are not merging correctly
ALL.D145$category <- gsub(" GO:","GO:",ALL.D145$category ) #remove extra characters
str(ALL.D145)
str(GO.annot)

ALL.D145 <- merge(ALL.D145,GO.annot, by="category" )
ALL.D145 <- ALL.D145[order(-ALL.D145$numDEInCat),]
str(ALL.D145)
MF.D145 <- subset(ALL.D145, namespace=="MF")
MF.D145 <- MF.D145[order(-MF.D145$numDEInCat),]
CC.D145 <- subset(ALL.D145, namespace=="CC")
CC.D145 <- CC.D145[order(-CC.D145$numDEInCat),]
BP.D145 <- subset(ALL.D145, namespace=="BP")
...
heatmap.2(Time.Amb.mat, margins = c(10,20), cexRow = 1.2, cexCol = 1, Colv=FALSE, dendrogram="row", labRow=FALSE, col = rev(RColorBrewer::brewer.pal(name = "RdYlBu", n = 10)), na.color = "black", density.info = "none", trace = "none", scale = "row", sepwidth=c(0.025,0.025), sepcolor="darkgray", keysize = 1)
dev.off()
...
heatmap.2(D10.mat, margins = c(10,20), cexRow = 1.2, cexCol = 1, Colv=FALSE, dendrogram="row", labRow=FALSE, col = rev(RColorBrewer::brewer.pal(name = "RdYlBu", n = 10)), na.color = "black", density.info = "none", trace = "none", scale = "row", sepwidth=c(0.025,0.025), sepcolor="darkgray", keysize = 1)
dev.off()
...
heatmap.2(D135.mat, margins = c(10,20), cexRow = 1.2, cexCol = 1, Colv=FALSE, dendrogram="row", labRow=FALSE, col = rev(RColorBrewer::brewer.pal(name = "RdYlBu", n = 10)), na.color = "black", density.info = "none", trace = "none", scale = "row", sepwidth=c(0.025,0.025), sepcolor="darkgray", keysize = 1)
dev.off()
```
...
heatmap.2(D145.mat, margins = c(10,20), cexRow = 1.2, cexCol = 1, Colv=FALSE, dendrogram="row", labRow=FALSE, col = rev(RColorBrewer::brewer.pal(name = "RdYlBu", n = 10)), na.color = "black", density.info = "none", trace = "none", scale = "row", sepwidth=c(0.1,0.1), sepcolor="white", keysize = 1, colsep=c(0,2,4,6))
dev.off()
...
```{r}
##### Gene Ontology Plots
MFplot <- MF.D145 %>% mutate(name = fct_reorder(name, numDEInCat)) %>%
  filter(numDEInCat > 1) %>%
  ggplot( aes(x=name, y=numDEInCat) ) +
      geom_segment( aes(x=name ,xend=name, y=0, yend=numDEInCat), color="black") +
...
BPplot <- BP.D145 %>% mutate(name = fct_reorder(name, numDEInCat)) %>%
  filter(numDEInCat > 2) %>%
  ggplot( aes(x=name, y=numDEInCat) ) +
      geom_segment( aes(x=name ,xend=name, y=0, yend=numDEInCat), color="black") +
      geom_point(size=1, color="black") +
  ...
  theme(panel.border = element_blank(), # Set border
        ...
        theme(axis.text = element_text(size=8))#Set the plot background
BPplot
...
# 
# #Day 135 
# for (i in 1:nrow(D135.chondrocyte.genes)){
#   sig.gene <- subset(MD.D135, gene==D135.chondrocyte.genes$gene[i])
#   means <- aggregate(per.meth ~ Initial.Treatment*gene, data=sig.gene, FUN=mean)
#   ses <- aggregate(per.meth ~ Initial.Treatment*gene, data=sig.gene, FUN=std.error)
#   means$ses <- ses$per.meth
# 
...
#     theme_bw()
# 
#   ggsave(temp_plot, file=paste0("RAnalysis/Output/Chondrocyte_D135plot_", D135.chondrocyte.genes$gene[i],"_",D135.chondrocyte.genes$protein.name.short[i],".png"), width = 14, height = 10, units = "cm")
# }
# 
# #Day 145 
...
# 
# #Day 145 
# for (i in 1:nrow(D145.chondrocyte.genes)){
#   sig.gene <- subset(MD.D145, gene==D145.chondrocyte.genes$gene[i])
#   means <- aggregate(per.meth ~ Initial.Treatment*gene, data=sig.gene, FUN=mean)
#   ses <- aggregate(per.meth ~ Initial.Treatment*gene, data=sig.gene, FUN=std.error)
#   means$ses <- ses$per.meth
# 
...
#     theme_bw()
# 
#   ggsave(temp_plot, file=paste0("RAnalysis/Output/Chondrocyte_D145plot_", D145.chondrocyte.genes$gene[i],"_",D145.chondrocyte.genes$protein.name.short[i],".png"), width = 14, height = 10, units = "cm")
# }
# 
# ```
