...
for (i in 1:nrow(sig.GO.Time)){
  sig.GO.Time$genes[i] <- paste(unname(Time.sig.annot[grep(sig.GO.Time$GO.IDs[i],Time.sig.annot$GO.IDs),"gene"]), collapse = "; ")
  sig.GO.Time$uniprot[i] <- paste(unname(Time.sig.annot[grep(sig.GO.Time$GO.IDs[i],Time.sig.annot$GO.IDs),"uniprot.id"]), collapse = "; ")
  sig.GO.Time$uniprot_taxa.id[i] <- paste(unname(Time.sig.annot[grep(sig.GO.Time$GO.IDs[i],Time.sig.annot$GO.IDs),"uniprot_taxa.id"]), collapse = "; ")
  sig.GO.Time$uniprot.name[i] <- paste(unname(Time.sig.annot[grep(sig.GO.Time$GO.IDs[i],Time.sig.annot$GO.IDs),"Uniprot.Name"]), collapse = "; ")
}
...
Time.GO.Sig <- arrangeGrob(TimeGO.BP,TimeGO.MF, ncol=2)
ggsave(file="RAnalysis/Figs/TimeGO.pdf", Time.GO.Sig, width = 15, height = 10, units = c("in"))

TimeGO <- ggplot(na.omit(sig.GO.Time), aes(x = ontology, y = term)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ ., scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
  ...
  strip.text.x = element_text(size = 20),
  axis.text = element_text(size = 8))
ggsave(file="RAnalysis/Figs/TimeGO_BPMF.pdf", TimeGO, width = 15, height = 10, units = c("in"))
...
for (i in 1:nrow(sig.GO.D10)){
  sig.GO.D10$genes[i] <- paste(unname(D10.sig.annot[grep(sig.GO.D10$GO.IDs[i],D10.sig.annot$GO.IDs),"gene"]), collapse = "; ")
...
D10.GO.Sig <- arrangeGrob(D10GO.BP,D10GO.MF, ncol=2)
ggsave(file="RAnalysis/Figs/GO.D10.pdf", D10.GO.Sig, width = 15, height = 10, units = c("in"))
...
for (i in 1:nrow(sig.GO.D135)){
  sig.GO.D135$genes[i] <- paste(unname(D135.sig.annot[grep(sig.GO.D135$GO.IDs[i],D135.sig.annot$GO.IDs),"gene"]), collapse = "; ")
...
sig.GO.D135$term <- factor(sig.GO.D135$term, levels = sig.GO.D135$term[nrow(sig.GO.D135):1])

#plot significantly enriched GO terms by Slim Category
D135GO <-  ggplot(sig.GO.D135, aes(x = ontology, y = term)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ ., scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
ggsave(file="RAnalysis/Figs/GO.D135.pdf", D135GO, width = 7, height = 12, units = c("in"))
```
...
##### GO enrichment of DMGs #####
DMG.D145 <- as.character(D145.sig$gene) #set the enrichment test list
...
write.csv(GO.D145 , file = "RAnalysis/Output/GO.D145.csv")
...
for (i in 1:nrow(sig.GO.D145)){
  sig.GO.D145$genes[i] <- paste(unname(D145.sig.annot[grep(sig.GO.D145$GO.IDs[i],D145.sig.annot$GO.IDs),"gene"]), collapse = "; ")
  sig.GO.D145$uniprot[i] <- paste(unname(D145.sig.annot[grep(sig.GO.D145$GO.IDs[i],D145.sig.annot$GO.IDs),"uniprot.id"]), collapse = "; ")
  sig.GO.D145$uniprot_taxa.id[i] <- paste(unname(D145.sig.annot[grep(sig.GO.D145$GO.IDs[i],D145.sig.annot$GO.IDs),"uniprot_taxa.id"]), collapse = "; ")
  sig.GO.D145$uniprot.name[i] <- paste(unname(D145.sig.annot[grep(sig.GO.D145$GO.IDs[i],D145.sig.annot$GO.IDs),"Uniprot.Name"]), collapse = "; ")
}
...
sig.GO.D145$term <- factor(sig.GO.D145$term, levels = sig.GO.D145$term[nrow(sig.GO.D145):1])

#plot significantly enriched GO terms by Slim Category
D145GO <-  ggplot(sig.GO.D145, aes(x = ontology, y = term)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ ., scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
ggsave(file="RAnalysis/Figs/GO.D145.pdf", D145GO, width = 7, height = 12, units = c("in"))


D145.sig.annot[grep(as.character(sig.GO.D145[17,1]),D145.sig.annot$GO.IDs), ]

```

Plotting all BP
```{r}
DEG.GO.Enrichment <- arrangeGrob(D10GO,D135GO,D145GO, ncol=3)
ggsave(file="RAnalysis/Figs/Figx_DEG.GO.Enrichment.pdf", DEG.GO.Enrichment, width = 21, height = 12, units = c("in"))
ggsave(file="RAnalysis/Figs/Figx_DEG.GO.Enrichment.png", DEG.GO.Enrichment, width = 21, height = 12, units = c("in"))
...
# nrow(sig.GOslim.Time)
# Time.slim.genes.1 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.Time[1,1]) #kinase activity
# Time.slim.genes.2 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.Time[2,1]) #unknown
# Time.slim.genes.3 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.Time[3,1]) #cytoskeleton
# Time.slim.genes.4 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.Time[4,1]) #other metabolic processes
# 
# #Day 10
# nrow(sig.GOslim.D10)
# D10.slim.terms.1 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[1,1]) #protein metabolism
# D10.slim.terms.2 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[2,1]) #other molecular function
# D10.slim.terms.3 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[3,1]) #transport
# D10.slim.terms.4 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[4,1]) #cytoskeleton
# D10.cytoskel <- unique(D10.slim.terms.4 %>% filter(as.character(D10.slim.terms.4$gene) %in% D10.sig.annot$gene))
# D10.cytoskel <- left_join(D10.cytoskel, D10.sig.annot, by="gene")
# 
# D10.slim.terms.5 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[5,1]) #other biological processes
# D10.slim.terms.6 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[6,1]) #developmental processes
# D10.slim.terms.7 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[7,1]) #other cellular component
# D10.slim.terms.8 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[8,1]) #signal transduction
# D10.slim.terms.9 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[9,1]) #other membranes
# D10.slim.terms.10 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D10[10,1]) #plasma membrane
# 
# write.csv(D10.cytoskel, file='RAnalysis/Output/D10.genes.of.interest.csv')
# 
# #Day 135
# nrow(sig.GOslim.D135)
# D135.slim.genes.1 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D135[1,1]) #cytoskeletal activity
# D135.cytoskel.act <- unique(D135.slim.genes.1 %>% filter(as.character(D135.slim.genes.1$gene) %in% D135.sig.annot$gene))
# D135.cytoskel.act <- left_join(D135.cytoskel.act, D135.sig.annot, by="gene")
# D135.slim.genes.2 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D135[2,1]) #cytoskeleton
# D135.cytoskel <- unique(D135.slim.genes.2 %>% filter(as.character(D135.slim.genes.2$gene) %in% D135.sig.annot$gene))
# D135.cytoskel <- left_join(D135.cytoskel, D135.sig.annot, by="gene")
# D135.slim.genes.3 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D135[3,1]) #cell-cell signaling
# D135.slim.genes.4 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D135[4,1]) #nucleic acid binding activity
# D135.nuc_acid_bind <- unique(D135.slim.genes.4 %>% filter(as.character(D135.slim.genes.4$gene) %in% D135.sig.annot$gene))
# D135.nuc_acid_bind <- left_join(D135.nuc_acid_bind, D135.sig.annot, by="gene")
# D135.slim.genes.5 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D135[5,1]) #translational apparatus
# 
# D135.goi <- rbind(D135.cytoskel.act,D135.cytoskel,D135.nuc_acid_bind)
# write.csv(D135.goi, file='RAnalysis/Output/D135.genes.of.interest.csv')
# 
# #Day 145
# nrow(sig.GOslim.D145)
# D145.slim.genes.1 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[1,1]) #unknown
# D145.slim.genes.2 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[2,1]) #other cellular component
# D145.slim.genes.3 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[3,1]) #other metabolic processes
# D145.slim.genes.4 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[4,1]) #nucleic acid binding activity
# D145.nuc_acid_bind <- unique(D145.slim.genes.4 %>% filter(as.character(D145.slim.genes.4$gene) %in% D145.sig.annot$gene))
# D145.nuc_acid_bind <- left_join(D145.nuc_acid_bind, D145.sig.annot, by="gene")
# D145.slim.genes.5 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[5,1]) #mitochondrion
# D145.slim.genes.6 <- GO.slimmed.terms %>% filter(GO.Slim.Term == sig.GOslim.D145[6,1]) #translational apparatus
# 
# D145.goi <- D145.nuc_acid_bind
# write.csv(D145.goi, file='RAnalysis/Output/D145.genes.of.interest.csv')
...
#### GO enrichment of DMGs #####
DMG.Persist <- as.character(ints_10_135 $gene) #set the enrichment test list
...
sig.GO.Persist$term <- factor(sig.GO.Persist$term, levels = sig.GO.Persist$term[nrow(sig.GO.Persist):1])

#plot significantly enriched GO terms by Slim Category
PersistGO <-  ggplot(sig.GO.Persist, aes(x = ontology, y = term)) + 
  geom_tile(aes(fill =over_represented_pvalue)) + 
  facet_grid(GO.Slim.Term ~ ., scales = "free_y", labeller = label_wrap_gen(width = 5, multi_line = TRUE))+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
...
ggsave(file="RAnalysis/Figs/GO.Persist.pdf", PersistGO, width = 7, height = 12, units = c("in"))





ints_10_135.annot[grep(as.character(sig.GO.Persist[1,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[2,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[3,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[4,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[5,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[6,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[7,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[8,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[9,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[10,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[11,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[12,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[13,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[14,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[15,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[16,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[17,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[18,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[19,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[20,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[21,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[22,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[23,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[24,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[25,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[26,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[27,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[28,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[29,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[30,1]),ints_10_135.annot$GO.IDs), ]
ints_10_135.annot[grep(as.character(sig.GO.Persist[31,1]),ints_10_135.annot$GO.IDs), ]
...
Persist.topBPGO.genes.D10 <- D10.sig.annot[grep(as.character(sig.GO.Persist[1,1]),D10.sig.annot$GO.IDs), ]
Persist.topBPGO.genes.D135 <- D135.sig.annot[grep(as.character(sig.GO.Persist[1,1]),D135.sig.annot$GO.IDs), ]
Persist.topBPGO.genes.D145 <- D145.sig.annot[grep(as.character(sig.GO.Persist[1,1]),D145.sig.annot$GO.IDs), ]

Persist.0048813 <- left_join(Persist.topBPGO.genes.D145, Persist.topBPGO.genes.D10,Persist.topBPGO.genes.D135, by="gene", keep = FALSE)

#GO:0048813
#Persist.0048813$gene[1]
#Persist.0048813$gene[2]

D10.Pgene <- MD.D10 %>%	
  filter(gene== Persist.0048813$gene[2])
D135.Pgene <- MD.D135 %>%	
  filter(gene== Persist.0048813$gene[2])
D145.Pgene <- MD.D145 %>%	
  filter(gene== Persist.0048813$gene[2])

Pgene <- rbind(D10.Pgene,D135.Pgene,D145.Pgene)

D10.PDMG <- Pgene %>%	
  filter(TimePoint=="Day10") %>%
  group_by(Initial.Treatment, Sample.ID) %>%
  summarise(m = mean(per.meth)) %>%
  group_by(Initial.Treatment) %>%
  summarise(mean = mean(m), sem = sd(m)/(sqrt(4))) %>%
ggplot(., aes(x=Initial.Treatment, y=mean, colour=Initial.Treatment, shape=Initial.Treatment)) +
geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.2, colour="black") +
geom_point(size=4) + #set point characteristics
scale_colour_manual(values = c("blue", "purple", "red")) + #set line color
xlab("Treatment") + #plot x axis label
ylab("Percent Methylation") + #plot y axis label
ylim(0,100)+ #set y limits to 0 and 100%
...
D145.PDMG <- Pgene %>%	
  filter(TimePoint=="Day145") %>%
  group_by(Initial.Treatment, Secondary.Treatment,Sample.ID) %>%
  summarise(m = mean(per.meth)) %>%
  group_by(Initial.Treatment, Secondary.Treatment) %>%
  summarise(mean = mean(m), sem = sd(m)/(sqrt(4))) %>%
ggplot(., aes(x=Secondary.Treatment, y=mean, group=Initial.Treatment, colour=Initial.Treatment, shape=Initial.Treatment)) + #plot data
    geom_line(size=0.7, position=position_dodge(.1)) + #plot lines
    scale_colour_manual(values = c("blue", "purple", "red")) + #set line color
    geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), #plot error bars
    width=0, position=position_dodge(.1), colour="black") + #set error bar characteristics
    geom_point(size=4, position=position_dodge(.1)) + #set point characteristics
    xlab("Secondary Treatment") + #plot x axis label
    ylab("Percent Methylation") + #plot y axis label
    theme_bw()

PDMG <- arrangeGrob(D10.PDMG,D135.PDMG,D145.PDMG, ncol=3)
ggsave(file="RAnalysis/Figs/PDMG.pdf", PDMG, width = 10, height = 4, units = c("in"))
...
```{r}
# D10.topBPGO.genes.1 <- D10.sig.annot[grep(as.character(BP.D10[1,1]),D10.sig.annot$GO.IDs), ]
# D135.topBPGO.genes.1 <- D135.sig.annot[grep(as.character(BP.D135[1,1]),D135.sig.annot$GO.IDs), ]
# D145.topBPGO.genes.1 <- D145.sig.annot[grep(as.character(BP.D145[1,1]),D145.sig.annot$GO.IDs), ]
# 
# D10.topBPGO.genes.2 <- D10.sig.annot[grep(as.character(BP.D10[2,1]),D10.sig.annot$GO.IDs), ]
# D135.topBPGO.genes.2 <- D135.sig.annot[grep(as.character(BP.D135[2,1]),D135.sig.annot$GO.IDs), ]
# D145.topBPGO.genes.2 <- D145.sig.annot[grep(as.character(BP.D145[2,1]),D145.sig.annot$GO.IDs), ]
# 
# D10.topBPGO.genes.3 <- D10.sig.annot[grep(as.character(BP.D10[3,1]),D10.sig.annot$GO.IDs), ]
# D135.topBPGO.genes.3 <- D135.sig.annot[grep(as.character(BP.D135[3,1]),D135.sig.annot$GO.IDs), ]
# D145.topBPGO.genes.3 <- D145.sig.annot[grep(as.character(BP.D145[3,1]),D145.sig.annot$GO.IDs), ]
