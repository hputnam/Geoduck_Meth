...
carb.output <- cbind(SW.chem$Measure.Date,  SW.chem$Sample.ID,  SW.chem$Treatment, SW.chem$Exposure, carb.output) #combine the sample information with the seacarb output
colnames(carb.output) <- c("Date",  "Tank",  "Treatment",	"Exposure.Period", "Salinity",	"Temperature", "pH",	"CO2",	"pCO2","HCO3",	"CO3",	"DIC", "TA",	"Aragonite.Sat") #Rename columns to describe contents
...
carb.output.E1 <- subset(carb.output, Exposure.Period=="Exposure1")
carb.output.E1$Treatment <- factor(carb.output.E1$Treatment)

pdf("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Water_Chem_withTA_Exposure1.pdf")
par(mfrow=c(3,2))
plot(carb.output.E1$Treatment, carb.output.E1$Temperature, xlab="Treatment", ylab="Temperature°C", ylim=c(11,20))
plot(carb.output.E1$Treatment, carb.output.E1$pH, xlab="Treatment", ylab="pH Total Scale", ylim=c(7.0,8.0))
plot(carb.output.E1$Treatment, carb.output.E1$pCO2, xlab="Treatment", ylab="pCO2 µmol kg-1", ylim=c(350,7000))
plot(carb.output.E1$Treatment, carb.output.E1$Salinity, xlab="Treatment", ylab="Salinity psu", ylim=c(24,29))
plot(carb.output.E1$Treatment, carb.output.E1$TA, xlab="Treatment", ylab="Total Alkalinity µmol kg-1", ylim=c(2600,3200))
plot(carb.output.E1$Treatment, carb.output.E1$Aragonite.Sat, xlab="Treatment", ylab="Aragonite Saturation State", ylim=c(0,4))
dev.off()

carb.output.E2 <- subset(carb.output, Exposure.Period=="Exposure2")
carb.output.E2$Treatment <- factor(carb.output.E2$Treatment)

pdf("~/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Output/Water_Chem_withTA_Exposure2.pdf")
par(mfrow=c(3,2))
plot(carb.output.E2$Treatment, carb.output.E2$Temperature, xlab="Treatment", ylab="Temperature°C", ylim=c(11,20))
plot(carb.output.E2$Treatment, carb.output.E2$pH, xlab="Treatment", ylab="pH Total Scale", ylim=c(7.0,8.0))
plot(carb.output.E2$Treatment, carb.output.E2$pCO2, xlab="Treatment", ylab="pCO2 µmol kg-1", ylim=c(350,7000))
plot(carb.output.E2$Treatment, carb.output.E2$Salinity, xlab="Treatment", ylab="Salinity psu", ylim=c(24,29))
plot(carb.output.E2$Treatment, carb.output.E2$TA, xlab="Treatment", ylab="Total Alkalinity µmol kg-1", ylim=c(2600,3200))
plot(carb.output.E2$Treatment, carb.output.E2$Aragonite.Sat, xlab="Treatment", ylab="Aragonite Saturation State", ylim=c(0,4))
dev.off()
