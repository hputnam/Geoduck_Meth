...
#Takes the output files from analysis in MACAU Lea et al 2015

#Day10 results
day10 <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day10/Day10-10xCov-results.txt", header=T, sep="")
length(which(day10$pvalue<0.05))
day10$qval <- qvalue(day10$pvalue)$qvalues
length(which(day10$qval<0.1))
day10.fdr.cor <- subset(day10, qval<0.1)
sig.10 <- subset(day10, pvalue<=0.05)

#Day135 results
day135 <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day135/Day135-10xCov-results.txt", header=T, sep="")
length(which(day135$pvalue<0.05))
day135$qval <- qvalue(day135$pvalue)$qvalues
length(which(day135$qval<0.1))
day135.fdr.cor <- subset(day135, qval<0.1)
sig.135 <- subset(day135, pvalue<=0.05)

#Day145 results
day145 <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day145/Day145-Secondary-Exposure-Predictor-results.txt", header=T, sep="")
length(which(day145$pvalue<0.05))
day145$qval <- qvalue(day145$pvalue)$qvalues
length(which(day145$qval<0.1))
day145.fdr.cor <- subset(day145, qval<0.1)
sig.145 <- subset(day145, pvalue<=0.05)

sig.10.135 <- merge(sig.10, sig.135, by="id") 
sig.10.145 <- merge(sig.10, sig.145, by="id") 
sig.135.145 <- merge(sig.135, sig.145, by="id") 


day10.meth <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day10/Day10-10xCov-macau-methcounts.txt", header=T, sep="")
day10.meth$meth.sum <- rowSums(day10.meth[,2:13])
day10.meth <-day10.meth[,c(1,14)]
day135.meth <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day135/Day135-10xCov-macau-methcounts.txt", header=T, sep="")
day135.meth$meth.sum <- rowSums(day135.meth[,2:13])
day135.meth <-day135.meth[,c(1,14)]
day145.meth <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day145/Day145-10xCov-Secondary-Exposure-Predictor-macau-methcounts.txt", header=T, sep="")
day145.meth$meth.sum <- rowSums(day145.meth[,2:25])
day145.meth <-day145.meth[,c(1,26)]
colnames(day145.meth) <- c("id", "meth.sum") 

circ.data <- merge(day10.meth, day135.meth, by="id", all=T)
circ.data <- merge(circ.data, day145.meth, by="id", all=T)
colnames(circ.data) <- c("id", "Day10", "Day135", "Day145")
circ.data[is.na(circ.data)] <- 0
circ.data <- circ.data[rowSums(circ.data[,2:4])!=0, ] 

#split string id
#unique id

#xlim <- c(min(circ.10$meth.sum), max(circ.10$meth.sum))
...
mat <-  t(circ.data)
mat <-  mat[-1,]
rownames(mat) = c("Ambient", "Low", "Super.Low")
colnames(mat) <- circ.data$id
...
par(mar = c(1, 1, 1, 1))
grid.col = NULL
grid.col[colnames(mat)] = c("red", "green", "blue")
#grid.col[colnames(mat)] = "grey"
...
#Plot day 0 methylome
# RCircos approach RCircos (http://cran.r-project.org/web/packages/RCircos/index.html)

#coordinates, gene name, chromosome number, expression

#build dataframe of scaffold, start, end, expression

#create idogram values from draft genome

#bed <- read.csv(file="/Users/hputnam/MyProjects/Geoduck_Epi/project_juvenile_geoduck_OA/RAnalysis/Data/Macau_output/Day10/Day10-10xCov-DiffMethRegions.bed", header=F, sep="")
#colnames(bed) <- c("scaff", "start", "end", "value")
...
library(circlize)
par(mar = c(1, 1, 1, 1))
treatments <- c("Day10", "Day135", "Day145")
ID <- circ.data$id 
circos.par(points.overflow.warning = FALSE)
circos.initialize(factors = treatments, x=ID)
circos.trackPlotRegion(factors = factors, x=circ.data$id,
                       panel.fun = function(circ.data$id, y) 
                       ylim = c(min(circ.data[,c(2:4)]), max(circ.data[,c(2:4)])),
                       bg.col = c("red", "green", "blue", rep("grey", 6)),
                       bg.border = NA, track.height = 0.05)



#circos plot by group example
#sum and seperate by treatment to get factor
